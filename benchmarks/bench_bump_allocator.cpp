...
constexpr std::array<unsigned, 64> kParserPattern = {
    8,  16, 24, 32, 48, 64, 16, 24, 8,  32, 48, 24, 16, 64, 8,  32,
    24, 48, 16, 8,  64, 32, 24, 16, 48, 8,  32, 64, 16, 24, 48, 8,
    8,  24, 48, 16, 32, 64, 24, 8,  16, 48, 32, 64, 24, 8,  16, 32,
    48, 64, 24, 16, 8,  32, 48, 24, 16, 64, 8,  32, 24, 48, 16, 8,
};
...
void fillVector(std::vector<SmallRecord, BumpAllocator<SmallRecord>>& values, std::size_t count) {
  for (std::size_t i = 0; i < count; ++i) {
    values.push_back(
        SmallRecord{static_cast<std::uint32_t>(i), static_cast<std::uint32_t>(i ^ 0x55u), i});
  }
}
...
static void BM_VectorGrowth(benchmark::State& state) {
  ...
  state.SetBytesProcessed(state.iterations() *
                          static_cast<std::int64_t>(kVectorCount * elementCount * sizeof(T)));
}
